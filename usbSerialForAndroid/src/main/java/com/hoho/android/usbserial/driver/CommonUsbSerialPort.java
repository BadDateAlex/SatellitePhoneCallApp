...
public abstract class CommonUsbSerialPort implements UsbSerialPort {
    ...
    public byte[] ShiftLogicalLeft(byte []src,int i){
        i++;//i是当前index，需要从下一位开始左移
        while(i<src.length-1){
            src[i]=src[i+1];
            i++;
        }
        byte[] newArray = new byte[src.length-1];
        System.arraycopy(src, 0, newArray, 0, src.length-1);
        return newArray;
    }
...
//                    newArray[newArray.length-1]=0x0D;
                    ...
                    while(i<my_len)
                    {
                        if(tempBuffer[i]==0x5C&&tempBuffer[i+1]==0x72)
                        {
                            tempBuffer[i]=0X0D;
                            tempBuffer=ShiftLogicalLeft(tempBuffer,i);
                            my_len--;//替换之后少了一个元素
